...
from .base import BaseAIResponseMapper
from ..chunks import (
...
class AnthropicResponseMapper(BaseAIResponseMapper):
    def __init__(self):
    ...
    def map(self, chunk: Any) -> Optional[Dict[str, Any]]:
        ...
        if class_name == "StreamEvent" and isinstance(event, dict):
            ...
            if event_type == "content_block_start":
                ...
                if cb_type == "text":
                    return TextStartChunk(message_id=self.last_message_id).to_json()
                elif cb_type == "thinking":
                    return ThinkingStartChunk(message_id=self.last_message_id).to_json()
                    ...
                    return ToolCallChunk(
                        tool_name=tool_name,
                        tool_id=cb.get("id", ""),
                        input={}, # Changed from input_data to input to match Pydantic schema
                ...
                if delta_type == "thinking_delta":
                    ...
                    return ThinkingDeltaChunk(thinking=thinking, message_id=self.last_message_id).to_json()
                elif delta_type == "text_delta":
                    text = delta.get("text", "")
                    return TextDeltaChunk(text=text, message_id=self.last_message_id).to_json()
                elif delta_type == "signature_delta":
                    return ThinkingStopChunk(message_id=self.last_message_id).to_json()

            elif event_type == "content_block_stop":
                cb_type = self.block_types.get(index)
                if cb_type == "text":
                    return TextEndChunk(message_id=self.last_message_id).to_json()
                ...
                for block in raw_blocks:
                    ...
                    if block_dict.get("type") == "tool_use":
                        ...
                        return ToolCallChunk(
                            ...
                            input=input_data, # Changed from input_data to input
                            message_id=self.last_message_id
                        ).to_json()
        ...
        # --- 4. Process Results (Costs & Files) ---
            ...
            if chunk_dict.get("total_cost_usd"):
                return CostChunk(
                    total_cost_usd=chunk_dict.get("total_cost_usd"), 
                    message_id=self.last_message_id
                ).to_json()
