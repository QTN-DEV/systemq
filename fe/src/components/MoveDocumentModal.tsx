...
function MoveDocumentModal({ 
  ...
  const folderTree = useMemo((): DocumentItem[] => {
    const folderMap = new Map<string, DocumentItem & { children: DocumentItem[] }>()
    const rootFolders: (DocumentItem & { children: DocumentItem[] })[] = []
  ...
  // Note: getAllAccessibleFolders() already filters by edit permissions,
  // so all folders in the list are guaranteed to have edit access
  ...
  const renderFolderTree = (folders: (DocumentItem & { children: DocumentItem[] })[], level = 0): ReactElement[] => {
