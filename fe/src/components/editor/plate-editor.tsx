...
export type PlateEditorProps = {
  /**
   * When set, the editor deserializes this Markdown instead of the playground demo.
   * Pass `''` for an empty document.
   */
...
export const PlateEditor = React.forwardRef<PlateEditorHandle, PlateEditorProps>(
  function PlateEditor({ initialMarkdown, readOnly = false, className }, ref) {
    const isMarkdownMode = initialMarkdown !== undefined;
        ...
        ...(isMarkdownMode
          ? {
              value: (ed) => {
                ...
                return ed.getApi(MarkdownPlugin).markdown.deserialize(md);
              },
            }
          : { value: playgroundValue }),
      },
      isMarkdownMode ? [initialMarkdown] : []
    );
    ...
    return (
      <Plate editor={editor} readOnly={readOnly}>
        <EditorContainer
          className={cn(isMarkdownMode && 'min-h-0 flex-1', className)}
          variant={isMarkdownMode ? 'default' : undefined}
        >
          <Editor variant={isMarkdownMode ? 'default' : 'demo'} />
        </EditorContainer>

        {!isMarkdownMode ? <SettingsDialog /> : null}
      </Plate>
    );
  }
);
...
const playgroundValue = normalizeStaticValue([
  {
