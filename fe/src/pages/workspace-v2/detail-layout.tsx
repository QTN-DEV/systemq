...
function getActiveTabFromPath(basePath: string, pathname: string): "files" | "skills" | "chat" | "settings" | "workflows" {
  const rest = pathname.slice(basePath.length).replace(/^\/+/, "");
  const first = rest.split("/")[0] || "files";
  if (first === "chat" || first === "settings" || first === "skills" || first === "workflows") {
    return first;
  }
  return "files";
}

const tabLabel: Record<"files" | "skills" | "chat" | "settings" | "workflows", string> = {
  files: "Files",
  skills: "Skills",
  chat: "Chat",
  settings: "Settings",
    ...
    <div className={cn("flex h-full min-h-0 flex-col", className)}>
      <div className="flex flex-shrink-0 flex-col gap-2 border-b bg-background px-4 py-2.5">
        ...
        <nav className="bg-muted/60 flex w-fit max-w-full flex-wrap gap-0.5 rounded-lg p-1" aria-label="Workspace sections">
          ...
          <Link to={`${basePath}/workflows`} className={tabTriggerClass(activeTab === "workflows")}>
            Workflows
          </Link>
